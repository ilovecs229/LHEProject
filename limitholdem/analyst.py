...
if __name__ == '__main__':
    ...
    with open(s+'headsup3.json') as f:
        data = json.load(f)
    trunc = []
    for _ in range(10000):
        trunc.append(data[_])
    with open(s+'headsup3.json','w') as g:
            json.dump(trunc, g)
    a = Analyst(400, s+'headsup3.json')
    hole = [Card('S','A'),Card('C','A')]
    board = [Card('C','J'),Card('C','Q'),Card('C','K'),Card('H','A'),Card('D','A')]
    '''
    for i in range(0,6):
        print(a.card_parameters(hole,board[:5-i]))
    with open("C:/Users/eax20/Downloads/rlcard-master/rlcard-master/rlcard/games/limitholdem/headsup.json") as f:
        data = json.load(f)
    c = 0
    for elem in data:
        if a.parameters(elem)['lastround'] =='X':
            c+=1
    print(c)
    '''
    
    a.filter(s+'headsup3showdown.json')
    print("filter done")
    a.gda_parameters(s+'headsup3showdown.json',s+'showdowndata3.json')